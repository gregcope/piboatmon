...
def loadConfig():
    ...
    try:
        lastruntime = configP.get('main', 'lastruntime')
    except:
        lastruntime = False
...
def processSMS(sms):
    ...
    if 'send status' in _lowertxt:
        ...
        logging.info('send Status SMS recieved from: ' + str(sms[0]['Number']))
...
def setPowerOnDelay():
    ...
    if mopi is False:
        logging.error('mopi not initialised.  Cannot set power on delay to: '
                      + str(wakeInNSecs))
        return
